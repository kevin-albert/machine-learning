...
%   'sz'            - Output feature map dimensions
    ...
    assert(numel(input_shape) == 2 || numel(input_shape) == 3);
    ...
    if padded
        ...
        sz = [input_shape(1), input_shape(2), filters];
    else
        % Retain only the valid portion of the convolution
        sz = [input_shape(1:2) - kernel, filters-1]+1;
    end
    
    % Scale weights according to size of input
    in_features = input_shape(3) * sum(kernel);
